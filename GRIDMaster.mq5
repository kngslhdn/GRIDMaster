...
// v7.003 - DIRECTION LOCK + RECOVERY RE-ENTRY BLOCK
//================================================================================================//
#property strict
#property copyright "Copyright 2026, Jarvis"
#property version   "7.003"
...
int OnInit(){SymbolTrade=_Symbol;OrdersID=(MagicNumber==0)?101010:MagicNumber;HandleRSI=iRSI(SymbolTrade,PERIOD_CURRENT,RSIPeriod,PRICE_CLOSE);HandleMA=iMA(SymbolTrade,PERIOD_CURRENT,MAPeriod,0,MODE_SMA,PRICE_CLOSE);InitialBalance=AccountInfoDouble(ACCOUNT_BALANCE);PeakEquity=MathMax(InitialBalance,AccountInfoDouble(ACCOUNT_EQUITY));HighWaterMark=InitialBalance;ReductionActions=0;LastMidPrice=0.0;BuyDirectionLocked=false;SellDirectionLocked=false;HardProtectionActive=false;IsTerminated=false;AnomalyBrakeActive=false;CurrentState=STATE_NORMAL;if(HandleRSI==INVALID_HANDLE||HandleMA==INVALID_HANDLE){Print("[INIT] Indicator initialization failed");return INIT_FAILED;}PrintFormat("[INIT] GRIDMaster v7.003 | Balance=%.2f | Hard=%.2f%% | Soft=%.2f%% | Reduce=%.2f%% | Emergency=%.2f%%",InitialBalance,HardLimit(),SoftLimit(),ReduceLimit(),EmergencyLimit());return INIT_SUCCEEDED;}
...
void ManageProfitExit(){if(BuyOrders>0&&BuyProfits>=TargetProfitUSD){CloseSide(POSITION_TYPE_BUY,"BUY-TARGET");UpdateStatus();}if(SellOrders>0&&SellProfits>=TargetProfitUSD){CloseSide(POSITION_TYPE_SELL,"SELL-TARGET");UpdateStatus();}if(!UseTrailingProfit)return;if(BuyOrders>0&&BuyProfits>=TrailingStartUSD)MaxBuyProfitSeen=MathMax(MaxBuyProfitSeen,BuyProfits);if(SellOrders>0&&SellProfits>=TrailingStartUSD)MaxSellProfitSeen=MathMax(MaxSellProfitSeen,SellProfits);if(BuyOrders>0&&MaxBuyProfitSeen>=TrailingStartUSD&&BuyProfits>0&&BuyProfits<=MaxBuyProfitSeen-TrailingStopUSD){CloseSide(POSITION_TYPE_BUY,"BUY-TRAIL");UpdateStatus();}if(SellOrders>0&&MaxSellProfitSeen>=TrailingStartUSD&&SellProfits>0&&SellProfits<=MaxSellProfitSeen-TrailingStopUSD){CloseSide(POSITION_TYPE_SELL,"SELL-TRAIL");UpdateStatus();}}

void UpdateProtectionState(double dd){if(BuyOrders+SellOrders==0){CurrentState=STATE_NORMAL;ReductionActions=0;return;}if(dd>=EmergencyLimit()){CurrentState=STATE_EMERGENCY;return;}if(dd>=SurvivalDD){CurrentState=STATE_SURVIVAL;return;}if(dd>=ReduceLimit()){CurrentState=STATE_REDUCE;return;}if(dd>=SoftLimit()){CurrentState=STATE_MONITOR;return;}CurrentState=STATE_NORMAL;}

void RecoveryGovernor(double dd){if(BuyOrders+SellOrders==0)return;if(TimeCurrent()<FreezeGridUntil)return;if((TimeCurrent()-LastRecoveryAction)<RecoveryCooldownSec)return;double bl=BasketLoss(POSITION_TYPE_BUY),sl=BasketLoss(POSITION_TYPE_SELL);bool danger=bl>=MaxBasketLossUSD,active=dd>=RecoveryStartDD,aggressive=dd>=AggressiveRecoveryDD,survival=dd>=SurvivalDD;if(dd<ReduceLimit())return;if(survival){CurrentState=STATE_SURVIVAL;if(ReductionActions>=MaxReductionActions)return;if(bl>=SurvivalBasketLossUSD||sl>=SurvivalBasketLossUSD){ENUM_POSITION_TYPE side=(bl>=SurvivalBasketLossUSD&&bl>=sl)?POSITION_TYPE_BUY:POSITION_TYPE_SELL;if(ReduceLargestLosingSide(side,"SURVIVAL-REDUCE"))return;}return;}if((danger||active)&&(aggressive||danger)&&ReductionActions<MaxReductionActions){if(ReduceLargestLoser("RECOVERY"))return;}}
