...
// v7.003 change summary:
// 1) DD >= SoftLimit blocks ALL new grid entries.
// 2) Recovery reduction locks the reduced direction until the managed basket is flat.
// 3) Recovery/survival reduction also imposes a longer re-entry freeze.
// 4) A direction cannot be re-created while it is locked, preventing loss->re-entry loops.
// 5) Existing profit/protection engines remain intact.
...
bool MarketHealthy()
{
   ...
   if(AnomalyBrakeActive)
   {
      ...
      AnomalyBrakeActive=false;AnomalyUntil=0;
   }
   bool anomaly=false;
   ...
   LastMidPrice=mid;
   if(anomaly){AnomalyBrakeActive=true;AnomalyUntil=now+AnomalyCooldownSec;return false;}
   return true;
}
...
int OnInit()
{
   ...
   InitialBalance=AccountInfoDouble(ACCOUNT_BALANCE);PeakEquity=MathMax(InitialBalance,AccountInfoDouble(ACCOUNT_EQUITY));HighWaterMark=InitialBalance;ReductionActions=0;LastMidPrice=0.0;
   BuyDirectionLocked=false;SellDirectionLocked=false;HardProtectionActive=false;IsTerminated=false;AnomalyBrakeActive=false;CurrentState=STATE_NORMAL;
   if(HandleRSI==INVALID_HANDLE||HandleMA==INVALID_HANDLE){Print("[INIT] Indicator initialization failed");return INIT_FAILED;}
   PrintFormat("[INIT] GRIDMaster v7.003 | Balance=%.2f | Hard=%.2f%% | Soft=%.2f%% | Reduce=%.2f%% | Emergency=%.2f%%",InitialBalance,HardLimit(),SoftLimit(),ReduceLimit(),EmergencyLimit());
...
void OnTick()
{
   ...
   // v7.003: once DD reaches the soft threshold, do not add any new risk.
...
//================================================================================================//
// DIRECTION LOCK ENGINE
//================================================================================================//
void UpdateDirectionLocks()
{
   // A lock is a recovery circuit breaker. It is released only after that side is completely flat.
...
void RecoveryGovernor(double dd)
{
   ...
   // v7.003: at the soft DD threshold, risk is frozen. Do not churn losses merely to satisfy recovery.
   if(dd<ReduceLimit())return;
...
bool ReduceLargestLoser(string reason)
{
   ...
   LockDirection(type,reason);
   ReductionActions++;LastRecoveryAction=TimeCurrent();SetRecoveryFreeze();RecoveryActionThisTick=true;
   PrintFormat("[REDUCE] %s | ticket=%I64u | side=%s | lot=%.4f | loss=%.2f | action=%d/%d | freeze=%ds",reason,t,EnumToString(type),vol,loss,ReductionActions,MaxReductionActions,(int)MathMax(RecoveryFreezeSec,SURVIVAL_REENTRY_FREEZE_SEC));
   return true;
}
